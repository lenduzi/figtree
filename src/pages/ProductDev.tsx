...
export default function ProductDev() {
  ...
  const nowCount = ideas.filter((idea) => idea.roadmap === "Now").length;
  const nextCount = ideas.filter((idea) => idea.roadmap === "Next").length;
  const laterCount = ideas.filter((idea) => idea.roadmap === "Later").length;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          { label: "Now", count: nowCount, tone: "bg-emerald-500/10 text-emerald-700" },
          { label: "Next", count: nextCount, tone: "bg-sky-500/10 text-sky-700" },
          { label: "Later (Backlog)", count: laterCount, tone: "bg-muted/60 text-muted-foreground" },
        ].map((item) => (
          <Card key={item.label} className="border-border/80">
            <CardContent className="p-4 flex items-center justify-between">
              ...
              <Badge className={cn("rounded-full px-3 py-1 text-xs", item.tone)}>
                Roadmap
              </Badge>
            </CardContent>
          </Card>
